...
def add_button_operation():
    ...
    if name in order_dict[category].keys():
        tmsg.showinfo("Error", "Dear Customer, \n\n  Item already exist in your order")
        return
...
def cancel_button_operation():
    ...
    if len(names)==0:
        ...
        return
    ans = tmsg.askquestion("Cancel Order", "Dear Customer, \n\n  Are You Sure to Cancel Order?")
    if ans=="no":
        return
    order_tabel.delete(*order_tabel.get_children())
    for i in menu_category:
...
def update_button_operation():
    ...
    if name not in order_dict[category].keys():
        tmsg.showinfo("Error", "Dear Customer, \n\n  Item is not in your order list")
        return
...
def bill_button_operation():
    ...
    if not customerContact.get().isdigit():
        ...
        return   
    ans = tmsg.askquestion("Generate Bill", "Dear Customer, \n\n  Are You Sure to Generate Bill?")
    ans = "yes"
    if ans=="yes":
        bill = Toplevel()
        bill.title("Bill")
        ...
        st = "Indian Coffee House\nPimple Nadupuram, Kollam-411061\n\n"
...
def close_window():
    tmsg.showinfo("Thanks", "Thanks for using my software \nfully developed by Python\n\nTo learn more about my projects...\nVisit- www.alan4tech.com")
    root.destroy()
...
root.title("M A D E   P R O U D L Y   B Y   A L A N")
...
title_label = Label(title_frame, text="H O T E L   A N A N T H A P U R I   C O N T I N E N T A L", 
...
order_title_label = Label(order_frame, text="Place Your Order Here", 
                    font=("times new roman", 20, "bold"),bg = "lightgreen", fg="red")
